...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("WHU Dataset - Standalone Test")
    print("=" * 60)

    # 创建临时测试数据
    with tempfile.TemporaryDirectory() as tmpdir:
        print(f"\n[1/4] Creating test data in {tmpdir}...")
        ...
        print("\n[2/4] Creating train dataset...")
        train_dataset = WHUDataset(config, split='train')
        print(f"  [OK] Train dataset: {len(train_dataset)} samples")

        print("\n[3/4] Creating val dataset...")
        val_dataset = WHUDataset(config, split='val')
        ...
        print(f"       Data range: {train_dataset.get_data_range()}")
